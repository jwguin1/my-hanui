...
/**
 * 초음파 서술의 **정본**. 사이트에서 초음파를 설명하는 문장은 여기 하나다.
 *
 * 「MRI 가 못 보는 걸 초음파가 본다」는 구도를 쓰지 않는다 —
 * 검증되지 않는 우위 주장이라 AI 가 인용에서 걸러낸다.
 * 초음파의 고유 강점은 해상도가 아니라 **움직이면서 본다는 것**이다.
 *
 * 의학적 문안이므로 원장 승인 없이 고치지 않는다.
 */
const ULTRASOUND_NOTE =
...
const QUESTIONS = [
  ...
  },
  {
    ...
    // FAQ 답변은 짧게 유지한다. 세 문장짜리 답을 여기 넣으면 AI 가 이 블록만
    // 떼어 인용할 때 맥락 없이 초음파 설명이 튀어나온다.
    // 질문에는 짧은 답, 설명이 필요하면 아래 본문(ULTRASOUND_NOTE)을 인용하게 나눈다.
    answer:
      "검사 결과와 실제 증상이 맞지 않는 경우가 있습니다. 이럴 때는 이학적 검사로 원인 부위를 다시 확인합니다.",
  },
  {
    quote: "수술해야 한다고 들었어요",
        ...
        <div className="section-padding">
          <SectionReveal>
            <div className="mx-auto max-w-3xl">
              ...
              {/* 초음파 서술의 정본. FAQ 에는 짧은 답만 두고 설명은 여기 한 곳에 모은다 —
                  AI 가 질문에는 짧은 답을, 설명이 필요할 때는 이 문단을 인용하게 나눈 것이다. */}
              <p className="mt-8 text-[15px] leading-[1.9] text-muted">
                {ULTRASOUND_NOTE}
              </p>
            </div>
